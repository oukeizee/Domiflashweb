(function(){
  ...
  const FRESATTO_SCHEDULE_TEXT="2:00 a. m. – 12:00 a. m.";
  ...
  function isFresattoCard(card){
    const text=String(card?.textContent||"").toLocaleLowerCase("es");
    const id=String(card?.dataset?.restaurantId||card?.querySelector('[data-restaurant-id]')?.dataset?.restaurantId||"").trim();
    const number=String(card?.querySelector('.restaurant-number')?.textContent||'').trim();
    return text.includes(FRESATTO_NAME.toLocaleLowerCase("es"))||number==='03'||id===FRESATTO_ID;
  }
  function isFresattoMenu(){
    const name=String(window.currentRestaurant?.name||"").toLocaleLowerCase("es");
    const id=new URLSearchParams(window.location.search).get("id");
    return name===FRESATTO_NAME.toLocaleLowerCase("es")||String(id||"")===FRESATTO_ID;
  }
  function isFresattoOpenNow(){const now=new Date();const hour=now.getHours()+(now.getMinutes()/60);return hour>=FRESATTO_OPEN_HOUR&&hour<FRESATTO_CLOSE_HOUR;}
  function closedLabel(){return '<span class="closed-badge">Cerrado ahora</span>';}

  function patchFresattoSchedule(){
  ...
  function applyClosedState(){
    ...
    document.querySelectorAll('.restaurant-card').forEach(card=>{
      const button=card.querySelector('.restaurant-name-button');if(!button)return;
      const open=isFresattoCard(card)?fresattoOpen:globalOpen;
      card.classList.toggle('is-closed',!open);button.classList.toggle('is-closed',!open);button.disabled=!open;button.setAttribute('aria-disabled',String(!open));
      if(!open){button.setAttribute('title','Este restaurante está cerrado');if(!card.querySelector('.closed-badge')){const number=card.querySelector('.restaurant-number');if(number)number.insertAdjacentHTML('afterend',closedLabel());}}
      else{button.removeAttribute('title');card.querySelector('.closed-badge')?.remove();}
    });
    document.querySelectorAll('.restaurant-order-card').forEach(card=>{
      const open=isFresattoCard(card)?fresattoOpen:globalOpen;
      card.classList.toggle('is-closed',!open);card.disabled=!open;card.setAttribute('aria-disabled',String(!open));
      if(!open)card.setAttribute('title','Este restaurante está cerrado');else card.removeAttribute('title');
    });
    document.querySelectorAll('.menu-add').forEach(button=>{
      if(button.dataset.closedBaseLabel===undefined)button.dataset.closedBaseLabel=button.textContent.trim();
      const open=isFresattoMenu()?fresattoOpen:globalOpen;
      button.disabled=!open;
      if(!open){button.textContent='Restaurante cerrado';button.setAttribute('title','Este restaurante está cerrado');}
      else{button.textContent=button.dataset.closedBaseLabel;button.removeAttribute('title');}
    });
    patchFresattoSchedule();
  }
  ...
  function renderDirectoryFallback(filter=""){
    ...
    const safe=value=>{const div=document.createElement("div");div.textContent=value==null?"":String(value);return div.innerHTML;};grid.innerHTML=list.map((r,index)=>`<article class="restaurant-card reveal" data-restaurant-id="${String(r.id)}" style="animation-delay:${Math.min(index*.025,.35)}s"><div><span class="restaurant-number">${String(r.id).padStart(2,"0")}</span></div><button class="restaurant-name-button" type="button" onclick="window.location.href='restaurante.html?id=${encodeURIComponent(r.id)}'"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 3v7M3.5 3v5a2.5 2.5 0 0 0 5 0V3M6 10.5V21M17 3v18M17 3c2.2 1.7 3.5 4.2 3.5 7v1H17"/></svg><span>${safe(r.name)}</span><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 12h13M13 6l6 6-6 6"/></svg></button>${scheduleHtml}</article>`).join("");applyClosedState();
  }
