(function(){
  ...
  function isFresattoCard(card){const text=String(card?.textContent||"").toLocaleLowerCase("es");return text.includes(FRESATTO_NAME.toLocaleLowerCase("es"))||String(card?.querySelector('.restaurant-number')?.textContent||'').trim()==='03';}
  ...
  function patchFresattoSchedule(){
    document.querySelectorAll('.restaurant-card').forEach(card=>{
      ...
      if(close)close.innerHTML='<strong>Cierre:</strong> Ninguno';
    });
  }
