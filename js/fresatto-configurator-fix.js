(function(){
  ...
  function ensureModal(){
    ...
    modal.querySelector('.product-config-backdrop').onclick=close;
    modal.querySelector('#fixConfigConfirm').onclick=()=>{if(!active||!valid(active.product,active.state))return;addToCart(active.product,active.state);close();};
    return modal;
  }
  ...
  function render(){
    ...
    body.innerHTML=(active.product.customization?.steps||[]).map((step,i)=>{
      ...
      const opts=(step.choices||[]).map(raw=>{
        ...
        return `<button type="button" class="config-choice ${on?'selected':''}" data-step="${esc(step.id)}" data-value="${esc(c.name)}" ${disabled?'disabled':''}><span class="config-choice-check">${on?'✓':''}</span><span class="config-choice-main"><strong>${esc(c.name)}</strong>${Number(c.price)>0?`<small>+ ${money(c.price)}</small>`:''}</span></button>`;
      }).join('');
    ...
    body.querySelectorAll('.config-choice').forEach(button=>{
      const handler=e=>{
        e.preventDefault();
        e.stopPropagation();
  ...
  function addToCart(product,state){const text=summary(product,state),unit=price(product,state),key=`${product.id}|${text||'base'}`;const cart=window.__domiflashRestaurantCart||(window.__domiflashRestaurantCart=[]);const existing=cart.find(x=>x.key===key);if(existing)existing.qty+=1;else cart.push({...product,key,price:unit,customization:{...state},customizationText:text,qty:1});renderCart();}
  function renderCart(){const cart=window.__domiflashRestaurantCart||[],box=document.getElementById('cartItems'),badge=document.getElementById('cartBadge'),totalEl=document.getElementById('cartTotal'),order=document.getElementById('whatsappOrder');const count=cart.reduce((s,x)=>s+x.qty,0),total=cart.reduce((s,x)=>s+(Number(x.price)||0)*x.qty,0);if(badge)badge.textContent=count;if(totalEl)totalEl.textContent=money(total);if(order)order.disabled=!cart.length;if(!box)return;box.innerHTML=cart.length?cart.map(item=>`<div class="cart-row"><div class="cart-row-top"><span class="cart-row-name">${esc(item.name)}</span><span class="cart-row-price">${money(item.price*item.qty)}</span></div>${item.customizationText?`<div class="cart-customization">${esc(item.customizationText)}</div>`:''}<div class="cart-controls"><button type="button" data-fix-key="${esc(item.key)}" data-fix-delta="-1">−</button><span>${item.qty}</span><button type="button" data-fix-key="${esc(item.key)}" data-fix-delta="1">+</button></div></div>`).join(''):`<div class="empty-cart">Tu carrito está vacío.<br>Agrega productos de la carta.</div>`;box.querySelectorAll('[data-fix-key]').forEach(b=>b.onclick=()=>{const item=cart.find(x=>x.key===b.dataset.fixKey);if(!item)return;item.qty+=Number(b.dataset.fixDelta);if(item.qty<=0)window.__domiflashRestaurantCart=cart.filter(x=>x.key!==item.key);renderCart();});}
